...
if ( !class_exists( 'WPM_Order_Terms' ) ) {
/**
 * I Order Terms plugin main class.
 *
 * @since 1.0.0
 */
class WPM_Order_Terms
{
	const PLUGIN_NAME = 'I Order Terms';
	const PLUGIN_VERSION = '1.3.1';
	const WP_MIN_VERSION = '3.5';
	const PLUGIN_BASENAME = 'order-terms/order-terms.php';
	const PLUGIN_OPTIONS_PAGE = 'wpm_orderterms_general';
	const LANG_DOMAIN = 'wpm_orderterms';
	...
	public function __construct( $plugin_path, $plugin_url )
	{
		...
		if ( is_admin() ) {
			...
			if ( defined( 'DOING_AJAX' ) ) {
				...
				$option = get_option( 'wpm_orderterms.general', null );
				if ( isset( $option ) ) {
					add_option( 'wpm_orderterms_general', $option, '', 'yes' );
					delete_option( 'wpm_orderterms.general' );
				}
			}
		}
	} // end __construct
	...
	{
		//wp_mail('admin@webografica.com', 'activate', 'order');

		// single site installation
		WPM_Order_Terms::activate_partial();
	...
	public static function activate_partial()
	{
		...
		if ( !$wpdb->get_row( $sql_check ) ) {
			...
			if ( $wpdb->query( $sql_column ) === false ) {
				trigger_error( sprintf( __( '%s error: Unable to add column, a required database change.', WPM_Order_Terms::LANG_DOMAIN ), WPM_Order_Terms::PLUGIN_NAME ), E_USER_ERROR );
				return;
			}
	...
	public function after_setup_theme()
	{
		...
		$options = get_option( 'wpm_orderterms_general' );
		...
		$taxonomies = apply_filters( 'wpm_order_terms_taxonomies', $this->taxonomies );
	...
	{
		// filter taxonomies
		$taxonomies = apply_filters( 'wpm_order_terms_taxonomies', $this->taxonomies_registered );
		if ( is_array( $taxonomies ) ) {
	...
	public function admin_init()
	{
		...
		// register settings
		register_setting( self::PLUGIN_OPTIONS_PAGE, 'wpm_orderterms_general', array($this, 'settings_general_sanitize') );
	...
	public function settings_general_print()
	{
		...
		<div class="wrap">
			...
			<form method="post" action="options.php">
				...
				<?php do_settings_sections( 'wpm_orderterms_general' ); ?>
		...
		<fieldset>
			...
			foreach ( $taxonomies as $taxonomy ) {
				...
				<label for="<?php echo "taxonomy-{$taxonomy_name_attr}"; ?>" title="<?php echo esc_attr( $taxonomy->description ); ?>">
					<input id="<?php echo "taxonomy-{$taxonomy_name_attr}"; ?>" name="wpm_orderterms_general[taxonomies-sort][]" type="checkbox" value="<?php echo $taxonomy_name_attr; ?>"
			...
			<p class="description"><?php esc_html_e( "(Taxonomies set as sortable via 'wpm_order_terms_taxonomies' filter or 'register_taxonomy' function can't be unchecked from options.)", self::LANG_DOMAIN ); ?></p>
		 </fieldset>
	...
	public function admin_scripts()
	{
		...
		// load script only on taxonomy screen and when orderby is not selected empty(
		if ( empty($_GET['orderby'] ) && !empty( $taxonomy ) && in_array( $taxonomy, $this->taxonomies ) ) {
			// custom styles

            wp_register_style( 'wpm_orderterms_custom_order', $this->plugin_url . '/css/admin-i-order-terms.css', false, self::PLUGIN_VERSION );
			wp_enqueue_style( 'wpm_orderterms_custom_order' );

			// WP scripts
			wp_enqueue_script( 'jquery-ui-sortable' );

			// custom scripts
			wp_register_script( 'wpm_orderterms_custom_order', $this->plugin_url . '/js/admin-i-order-terms.js', array('jquery-ui-sortable'), self::PLUGIN_VERSION );
			wp_enqueue_script( 'wpm_orderterms_custom_order' );
		}
	} // end admin_scripts
