...
if ( ! class_exists( 'WPM_Page_Ordering' ) ) :

class WPM_Page_Ordering {

	/**
	 * Handles initializing this class and returning the singleton instance after it's been cached.
	 *
	 * @return null|WPM_Page_Ordering
	 */
	...
	public static function wp() {
		if ( 0 === strpos( get_query_var('orderby'), 'menu_order' ) ) {
			$script_name = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? 'wpm-post-ordering.dev.js' : 'wpm-post-ordering.js';
			wp_enqueue_script( 'simple-page-ordering', plugins_url( $script_name, __FILE__ ), array('jquery-ui-sortable'), '2.1', true );
			wp_enqueue_style( 'simple-page-ordering', plugins_url( 'wpm-post-ordering.css', __FILE__ ) );
		}
	}
	...
	public static function admin_head() {
		...
		$screen->add_help_tab(array(
			...
			'title' => 'Сортировка',
			'content' => '<p>' . __( 'Расположите материалы в нужной последовательности.', 'simple-page-ordering' ) . '</p>',
		));
	}
	...
	public static function ajax_simple_page_ordering() {
        ...
        wpm_update_rearranged_schedules($_POST['id']);
	...
	public static function sort_by_order_link( $views ) {
		...
		$views['byorder'] = sprintf('<a href="%s" class="%s">%s</a>', $query_string, $class, __("Сортировка", 'simple-page-ordering'));
...
WPM_Page_Ordering::get_instance();
