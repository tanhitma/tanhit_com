...
class WPM_Comment_Image {
	...
	private function __construct() {
		...
		//if(get_post_type() != 'wpm-page') return;
		...
		if( $this->can_save_files() ) {
			...
			add_action( 'wpm_head', array( $this, 'add_styles' ), 920 );
			add_action( 'wpm_head', array( $this, 'add_scripts' ), 920 );
	 ...
	 *---------------------------------------------*/
	 ...
	 function add_styles() {
         if(get_post_type() != 'wpm-page') return;
         wpm_enqueue_style( 'comment-images', plugins_url( '/member-luxe/plugins/comment-images/css/plugin.css' ) );
	...
	function add_scripts() {
        ...
        wpm_enqueue_script( 'comment-images', plugins_url( '/member-luxe/plugins/comment-images/js/plugin.min.js' ) );
	...
	function add_admin_styles() {
		...
		$screen = get_current_screen();
       	if( 'wpm-page' === $screen->id ) {
			wpm_enqueue_style( 'comment-images-admin', plugins_url( '/member-luxe/plugins/comment-images/css/admin.css' ) );
		} // end if
	...
	function add_admin_scripts() {
		...
		$screen = get_current_screen();
		if( 'wpm-page' === $screen->id ) {
			wp_enqueue_script( 'comment-images', plugins_url( '/member-luxe/plugins/comment-images/js/admin.min.js' ), array( 'jquery' ) );
		} // end if
	...
	function save_comment_image( $comment_id ) {
        ...
        if(count($_FILES[$comment_image_id])) {

            foreach ($_FILES[$comment_image_id]['name'] as $key => $filename) {
                ...
                if( $this->is_valid_file_type( $file_name_parts[ count( $file_name_parts ) - 1 ] ) ) {
                    ...
                    $comment_image_file = wp_upload_bits( $filename, null, file_get_contents( $_FILES[$comment_image_id]['tmp_name'][$key] ) );

					$file_array = array();
					$file_array['name'] = $filename;
					$file_array['tmp_name'] = $comment_image_file['file'];
					$id = media_handle_sideload( $file_array, $post_id );
				    $comment_image_file['url'] = wp_get_attachment_url( $id );
	 ...
	 *---------------------------------------------*/
	 ...
	 public function comment_images_display( $post ) {
		 ...
		 $comment_image_state = 'enabled'; //$comment_image_state = 'disabled';
