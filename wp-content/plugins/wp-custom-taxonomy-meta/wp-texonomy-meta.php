            ...
            foreach ($metaList as $inputName => $inputData) {
                ...
                if ($wp_version < '3.0' || $inputTaxonomy == $taxonomy) {
                    ...
                    <?php } elseif ($inputType == 'select') { 
						$aData = apply_filters('term_fields_select_'.$inputName);
					...
					?>
                        ...
                        <tr class="form-field">
                            ...
                            <td>
								<select name="<?php echo 'wpaft_'.$inputName;?>">
									<option value=''></option>
									<?foreach($aData as $aItem){?>
										<option value='<?=$aItem['val']?>'<?=($aItem['val'] == $inputValue ? ' selected="selected"' : '')?>><?=$aItem['label']?></option>
									<?}?>
								</select>
                            </td>
                        </tr>    
