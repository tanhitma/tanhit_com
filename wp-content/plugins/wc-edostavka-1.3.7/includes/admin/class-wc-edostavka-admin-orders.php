...
class WC_Edostavka_Admin_Orders {
	...
	public function register_metabox($post_type, $post) {
		if ( ! woo_order_has_virtual_product($post->ID)){
			add_meta_box(
				'wc_edostavka',
				'СДЕК ПВЗ',
	...
	public function metabox_content( $post ) {
		...
		$iMethod_id = str_replace( 'edostavka_', '', $shipping_method );
		
		$html = '<label for="delivery_point">Пункт выдачи</label><br />';
		if( sizeof( $points ) > 0 ) {
			$html .= '<select id="delivery_point" name="delivery_point">';
			foreach( $points as $point ) {
				if ((in_array($iMethod_id, array(301,302)) && $point['Type'] == 'POSTOMAT') || ($point['Type'] != 'POSTOMAT')){
	...
	public function save_delivery_points( $post_id ) {
		...
		if ( isset( $_POST['delivery_point'] ) && strpos( $shipping_method, 'edostavka_' ) === 0 ) {
			...
			if ( ( $new && $new != $old ) ) {
				...
				update_post_meta( $post_id, '_billing_delivery_point', $new );				
				...
				delete_post_meta( $post_id, '_billing_delivery_point', $old );
			}
		}
	}
