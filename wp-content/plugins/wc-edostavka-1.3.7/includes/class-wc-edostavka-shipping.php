...
class WC_Edostavka_Shipping_Method extends WC_Shipping_Method {
	...
	function is_available( $package ) {
		if ( $this->enabled == "no" ) return false;
                ...
                //С какой бы страны пользователь не был
                return true;
	...
	public function calculate_shipping( $package = array() ) {
		...
		if( empty($package['contents']) || ($this->enabled_in_cart !== 'yes' && is_cart()) ) return;

		$aNewPackage = array();
		foreach($package['contents'] as $sKey => $productItem){
			if ($productItem['data']->virtual == 'no'){
				$aNewPackage[$sKey] = $productItem;
			}
		}
		
		if ( ! $aNewPackage) return false;
		
		$package['contents'] = $aNewPackage;
		...
		if ( ! empty( $shipping_values ) ) {
			...
			if ($this->autoselect_edostavka_shipping_method === 'yes' && $chosen_method !== null && ! WC()->session->get( 'chosen_shipping_methods' )) {
