...
if (!class_exists('WC_Edostavka')) :
    ...
    class WC_Edostavka {
        ...
        private function __construct() {
            ...
            add_filter('woocommerce_checkout_fields', [$this, 'add_delivery_points_field'], 99);
            add_filter('woocommerce_default_address_fields', [$this, 'default_address_fields'], 99);
            add_filter('woocommerce_country_locale_field_selectors', [$this, 'add_city_id_field_selector']);
            add_filter('woocommerce_form_field_hidden', [$this, 'form_field_hidden'], 10, 4);

            add_action('woocommerce_after_checkout_billing_form', [$this, 'delivery_points_map']);
            add_action('woocommerce_checkout_process', [$this, 'delivery_point_field_process']);
            ...
            add_action('woocommerce_email_order_meta', [$this, 'email_order_meta'], 99);
            add_filter('default_checkout_state', [$this, 'default_checkout_state']);

            //add_filter( 'pre_update_option_woocommerce_' . self::$method_id . '_settings', array( $this, 'check_shop_contract' ), 10, 2 );

            add_filter('woocommerce_cart_shipping_packages', [$this, 'shipping_packages'], 10);

            //Ajax
            add_filter('woocommerce_update_order_review_fragments', [$this, 'ajax_update_delivery_points']);

            add_filter('woocommerce_params', [$this, 'is_door_params']);
        ...
        public function admin_load_script($hook) {
            if (urldecode($hook) !== 'магазин_page_wc-settings') return;
        ...
        public static function wc_edostavka_delivery_tariffs_type($type) {
            if (in_array($type, [1, 3, 11, 16, 18, 57, 58, 59, 60, 61, 137, 139, 233])) {
                return 'door';
            } else {
                return 'stock';
            }
        ...
        public function default_address_fields($fields) {
            ...
            $fields = array_merge($fields, [
                ...
                'delivery_point' => [
                    ...
                    'required' => false,
                    'class'    => ['form-row-wide', strpos(self::get_chosen_shipping_method(), self::get_method_id()) === 0 ? '' : 'hidden'],
                    'clear'    => true
                ],
                'state_id'       => [
                    'label'    => __('ID города'),
        ...
        public function ajax_update_delivery_points($fragments) {
            ...
            if (sizeof($delivery_points) > 0) {
                ...
                foreach ($delivery_points as $delivery_point) {
                    if ((in_array($iMethod_id, [301, 302]) && $delivery_point['Type'] == 'POSTOMAT') || ($delivery_point['Type'] != 'POSTOMAT')) {
        ...
        public function is_door_params($params) {
            $params['is_door'] = [1, 3, 11, 16, 18, 57, 58, 59, 60, 61, 137, 139, 233];
    ...
    add_action('plugins_loaded', ['WC_Edostavka', 'get_instance'], 0);
